...
export const VirtualMessageList = memo(({
  ...
  const getItemSize = useCallback((index: number) => {
    ...
    if (!isExpanded) return 40; // 紧凑视图高度
    
    // 展开时根据内容计算
    const valueLength = msg.value.length;
    const extraHeight = Math.min(Math.ceil(valueLength / 100) * 20 + 150, 500);
